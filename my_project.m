...
msgbox({'Hello...','Your model for Mushroom Classification Binary !!',...
...
% Define factor names
factor_names = {'Cap Diameter ',' Cap Shape ',' Gill Attachment ',' Gill Color ',' Stem Height ',' Stem Width ',' Stem Color ',' Season'};
...
if strcmp(choice, 'Select Row Number')
    ...
    prompt = {'Select the row number (2-54035) to make it test_data:'};
    dlgtitle = 'Input';
    dims = [1 50];
    definput = {'2'}; % Default input value
    ...
    if (test_row > 54036 || test_row < 2 )
        msgbox('Number is not in the range 2 : 54036', 'Invalid Input', 'error', 'modal');
    ...
    train_data = my_data([2:test_row-1 , test_row+1:end ], :);
    train_target = my_output([2:test_row-1 , test_row+1:end], :);
...
if strcmp(way, 'row')
    ...
    combined_msg = sprintf(['1 // Means poisonous , 0 // edible\n\n', ...
                        'Acual Class : %d\n\nFactors Information:\n\n'], my_output(test_row,:));
else
    % Displaying actual class and factors information in one message box
    combined_msg = sprintf(['1 // Means poisonous , 0 // edible\n\n', ...
...
msgbox(combined_msg, ' Prediction and Information');
pause(3);
...
if strcmp(way, 'row')
    if predict_class == 1
        if predict_class == my_output(test_row,:)
            msgbox('Predicted Class :  1 // Means poisonous /// result maches the acual class !!', 'Predicted Class Information');
        else
            msgbox('Predicted Class :  1// Means poisonous /// result not maches the acual class !!', 'Predicted Class Information');
        end
    else
        if predict_class == my_output(test_row,:)
            msgbox('Predicted Class :  0 // edible /// result maches the acual class !!', 'Predicted Class Information');
        else
            msgbox('Predicted Class :  0 // edible /// result not maches the acual class !!', 'Predicted Class Information');
        end
    end
else
    if predict_class == 1
        msgbox('Predicted Class :  1 // Means poisonous ', 'Predicted Class Information');
    else
        msgbox('Predicted Class :  0 // edible ', 'Predicted Class Information');
    end
end
